$.fn.Pushy = function(options) {
  ...
  // PUSHY INSTANCES
  var instanceNumber = ($.fn.Pushy.numInstances =
    ($.fn.Pushy.numInstances || 0) + 1);
  ...
  this.containerPush =
    options.containerPush == undefined ? true : options.containerPush;
  ...
  this.overlayShow =
    options.overlayShow == undefined ? true : options.overlayShow;
  ...
  var pushy = $(this.menu), //menu css class
    ...
    pushyClass =
      "pushy-" + this.menuPosition + " pushy-" + this.menuPosition + "-open", //menu position & menu open class
    ...
    containerClass =
      ...
      "-push" +
      " " +
      this.containerCustomClass; //container open class
    ...
    if (that.containerPush) {
      ...
      if (that.menuPosition == POSITION_TOP) {
        if (isOpenPushy()) {
          container.css(
            "transform",
            "translate3d(0," + pushy.height() + "px,0)"
          );
        } else {
